...
const SECTIONS = [
  ...
  },
  {
    ...
    content: [
      { type: 'text', value: '系統支援自動排程，可以設定每隔幾小時自動爬取一次。也可以隨時手動觸發。' },
      {
        type: 'tips',
        items: [
          '建議頻率：每 4～8 小時執行一次，確保資料即時反映市場變動',
  ...
  },
  {
    ...
    content: [
      { type: 'text', value: '「蝦皮追蹤」是主要功能區的獨立頁面，支援多組關鍵字同時追蹤，手動執行即可取得蝦皮商城的最新搜尋結果。' },
      {
        type: 'steps',
        items: [
          { title: '前往蝦皮追蹤頁', desc: '點擊左側主選單「蝦皮追蹤」，進入關鍵字管理頁面。' },
          { title: '新增追蹤關鍵字', desc: '在左上角輸入框輸入關鍵字（如「雪Q餅」、「精華液」），按 Enter 或點「+」新增到追蹤清單。' },
          { title: '執行抓取', desc: '點擊 tag 上的「↻」立即執行，執行完成後右側會自動顯示結果。' },
      ...
      },
      {
        type: 'tips',
        items: [
          '關鍵字建議輸入「品牌 + 品類」（如蘭蔻精華、SKII 神仙水），結果更精準',
          '若執行後結果筆數為 0，請嘗試更換關鍵字，或聯繫管理員確認系統狀態',
          '點擊商品卡片可直接跳轉至蝦皮商品頁',
        ],
      },
    ],
    ...
    content: [
      {
        ...
        items: [
          ...
          { q: '蝦皮可以追蹤幾個關鍵字？',     a: '關鍵字數量無限制，但建議一次管理 10 組以內，以確保每次執行的速度與結果品質。' },
