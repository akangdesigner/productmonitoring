...
router.get('/', (req, res) => {
  ...
  let query = `
    ...
    WHERE (p.user_id = ? OR p.id IS NULL)
  `;
  const params = [uid];
...
router.get('/gaps', (req, res) => {
  ...
  const gapRows = rows
    .map(r => {
      ...
      const latestAt = [r.watsons_at, r.cosmed_at, r.poya_at].filter(Boolean).sort().slice(-1)[0] || null;
      return {
        product_id: r.group_key, product_name: r.product_name, brand: r.brand,
        watsons_price: r.watsons_price ?? null,
        cosmed_price:  r.cosmed_price  ?? null,
        poya_price:    r.poya_price    ?? null,
        min_price: minPrice, max_price: maxPrice, gap, latest_at: latestAt,
      };
    })
    .filter(Boolean)
  ...
  res.json({
    ...
    items: gapRows.slice(offset, offset + limit),
  });
});
